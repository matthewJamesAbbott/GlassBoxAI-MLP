...
#
# Matthew Abbott 2025
# Test for both mlp_opencl.cpp and facaded_mlp_opencl.cpp
#
...
FAIL=0
TOTAL=0
TEMP_DIR="./output/mlp_opencl_user_tests_$$"
MLP_BIN="./MLP_opencl"
FACADE_BIN="./facaded_mlp_opencl"
...
g++ -o MLP_opencl mlp_opencl.cpp -lOpenCL
g++ -o facaded_mlp_opencl facaded_mlp_opencl.cpp -lOpenCL
...
echo ""
echo "========================================="
echo "MLP OpenCL User Workflow Test Suite"
echo "========================================="
echo ""

# Check binaries exist
if [ ! -f "$MLP_BIN" ]; then
    echo -e "${RED}Error: $MLP_BIN not found. Compile with: g++ -o MLP_opencl mlp_opencl.cpp -lOpenCL${NC}"
    exit 1
fi

if [ ! -f "$FACADE_BIN" ]; then
    echo -e "${RED}Error: $FACADE_BIN not found. Compile with: g++ -o facaded_mlp_opencl facaded_mlp_opencl.cpp -lOpenCL${NC}"
    exit 1
fi

echo -e "${BLUE}=== MLP OpenCL Binary Tests ===${NC}"
echo ""
...
run_test \
    "MLP OpenCL help command" \
    "$MLP_BIN help" \
    "Commands:"

run_test \
    "MLP OpenCL --help flag" \
    "$MLP_BIN --help" \
    "Commands:"

run_test \
    "FacadeMLP OpenCL help command" \
    "$FACADE_BIN help" \
    "Commands:"
...
run_test \
    ...
    "$MLP_BIN create --input=2 --hidden=3 --output=1 --save=$TEMP_DIR/sigmoid.json --output-act=sigmoid" \
    "Created MLP model"
...
run_test \
    "Create with linear output" \
    "$MLP_BIN create --input=2 --hidden=3 --output=1 --save=$TEMP_DIR/linear.json --output-act=linear" \
    "Created MLP model"
...
run_test \
    "Model info shows input size" \
    "$MLP_BIN info --model=$TEMP_DIR/basic.json" \
    "Input size"

run_test \
    "Model info shows output size" \
    "$MLP_BIN info --model=$TEMP_DIR/basic.json" \
    "Output size"

run_test \
    "Model info shows hidden sizes" \
    "$MLP_BIN info --model=$TEMP_DIR/basic.json" \
    "Hidden sizes"
...
echo -e "${BLUE}Group: Forward Pass / Prediction${NC}"

run_test \
    "Predict with 2 inputs for 2-4-1 network" \
    "$MLP_BIN predict --model=$TEMP_DIR/basic.json --input=0.5,0.5" \
    "Output:"

run_test \
    "Predict with multiple inputs for multi-layer" \
    "$MLP_BIN predict --model=$TEMP_DIR/multilayer.json --input=0.1,0.2,0.3" \
    "Output:"

run_test \
    "Prediction succeeds with valid input" \
    "$MLP_BIN predict --model=$TEMP_DIR/basic.json --input=0,0" \
    "Output:"

run_test \
    "Prediction succeeds with negative input" \
    "$MLP_BIN predict --model=$TEMP_DIR/basic.json --input=-0.5,-0.5" \
    "Output:"
...
# ============================================
# Weight Access (Facade)
# ============================================

echo -e "${BLUE}Group: Weight Access (Facade)${NC}"

run_test \
    "Get weight from layer 0" \
    "$FACADE_BIN get-weight --model=$TEMP_DIR/basic.json --layer=0 --neuron=0 --weight=0" \
    "Weight"

run_test \
    "Get weight from hidden layer" \
    "$FACADE_BIN get-weight --model=$TEMP_DIR/multilayer.json --layer=1 --neuron=0 --weight=0" \
    "Weight"

echo ""

# ============================================
# Weight Modification (Facade)
# ============================================

echo -e "${BLUE}Group: Weight Modification (Facade)${NC}"

run_test \
    "Set weight in layer 0" \
    "$FACADE_BIN set-weight --model=$TEMP_DIR/basic.json --layer=0 --neuron=0 --weight=0 --value=0.123 --save=$TEMP_DIR/modified.json" \
    "set-weight"

check_file_exists \
    "Modified model file created" \
    "$TEMP_DIR/modified.json"

run_test \
    "Modified model can be loaded" \
    "$MLP_BIN info --model=$TEMP_DIR/modified.json" \
    "MLP Model Information"

run_test \
    "Modified model can predict" \
    "$MLP_BIN predict --model=$TEMP_DIR/modified.json --input=0.5,0.5" \
    "Output:"

echo ""

# ============================================
# Bias Access and Modification (Facade)
# ============================================

echo -e "${BLUE}Group: Bias Access and Modification (Facade)${NC}"

run_test \
    "Get bias from layer 0" \
    "$FACADE_BIN get-bias --model=$TEMP_DIR/basic.json --layer=0 --neuron=0" \
    "Bias"

run_test \
    "Set bias in layer 0" \
    "$FACADE_BIN set-bias --model=$TEMP_DIR/basic.json --layer=0 --neuron=0 --value=0.5 --save=$TEMP_DIR/modified_bias.json" \
    "set-bias"

check_file_exists \
    "Model with modified bias created" \
    "$TEMP_DIR/modified_bias.json"

run_test \
    "Model with modified bias can predict" \
    "$MLP_BIN predict --model=$TEMP_DIR/modified_bias.json --input=0.5,0.5" \
    "Output:"

echo ""

# ============================================
# Batch Prediction (Facade)
# ============================================

echo -e "${BLUE}Group: Batch Prediction (Facade)${NC}"

run_test \
    "Batch predict with single input" \
    "$FACADE_BIN batch-predict --model=$TEMP_DIR/basic.json --input=0.5,0.5" \
    "Output:"

run_test \
    "Batch predict with multiple inputs" \
    "$FACADE_BIN batch-predict --model=$TEMP_DIR/multilayer.json --input=0.2,0.3,0.4" \
    "Output:"

echo ""

# ============================================
# Layer Information (Facade)
# ============================================

echo -e "${BLUE}Group: Layer Information (Facade)${NC}"

run_test \
    "Get layer 0 information" \
    "$FACADE_BIN layer-info --model=$TEMP_DIR/basic.json --layer=0" \
    "Layer 0"

run_test \
    "Get hidden layer information" \
    "$FACADE_BIN layer-info --model=$TEMP_DIR/multilayer.json --layer=1" \
    "Layer 1"
...
echo -e "${BLUE}Group: Layer Output (Facade)${NC}"

run_test \
    "Get layer output with input" \
    "$FACADE_BIN get-output --model=$TEMP_DIR/basic.json --layer=0 --neuron=0 --input=0.5,0.5" \
    "Output"

run_test \
    "Get hidden layer output" \
    "$FACADE_BIN get-output --model=$TEMP_DIR/multilayer.json --layer=1 --neuron=0 --input=0.1,0.2,0.3" \
    "Output"
...
echo -e "${BLUE}Group: Histogram (Facade)${NC}"

run_test \
    "Get activation histogram" \
    "$FACADE_BIN histogram --model=$TEMP_DIR/basic.json --layer=0 --type=activation" \
    "Histogram"

run_test \
    "Get weight histogram" \
    "$FACADE_BIN histogram --model=$TEMP_DIR/basic.json --layer=0 --type=weight" \
    "Histogram"
