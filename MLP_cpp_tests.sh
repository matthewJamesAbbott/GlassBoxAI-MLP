...
#
# Matthew Abbott 2025
# Test for both MLP.cpp and FacadeMLP.cpp
#
...
FAIL=0
TOTAL=0
TEMP_DIR="./output/mlp_cpp_tests_$$"
MLP_BIN="./MLP_cpp"
FACADE_BIN="./FacadeMLP_cpp"
...
g++ -o MLP_cpp MLP.cpp
g++ -o FacadeMLP_cpp FacadeMLP.cpp
...
echo ""
echo "========================================="
echo "MLP C++ User Workflow Test Suite"
echo "========================================="
echo ""

# Check binaries exist
if [ ! -f "$MLP_BIN" ]; then
    echo -e "${RED}Error: $MLP_BIN not found. Compile with: g++ -o MLP_cpp MLP.cpp${NC}"
    exit 1
fi

if [ ! -f "$FACADE_BIN" ]; then
    echo -e "${RED}Error: $FACADE_BIN not found. Compile with: g++ -o FacadeMLP_cpp FacadeMLP.cpp${NC}"
    exit 1
fi

echo -e "${BLUE}=== MLP C++ Binary Tests ===${NC}"
echo ""
...
echo -e "${BLUE}Group: Predictions${NC}"

run_test \
    "Single prediction on basic model" \
    "$MLP_BIN predict --model=$TEMP_DIR/basic.json --input=0.5,0.5" \
    "Output:"

run_test \
    "Prediction on multilayer model" \
    "$MLP_BIN predict --model=$TEMP_DIR/multilayer.json --input=0.1,0.2,0.3" \
    "Output:"

run_test \
    "Multiple predictions in batch" \
    "echo '0.5,0.5' | $MLP_BIN predict --model=$TEMP_DIR/basic.json --input=-" \
    "Output:"

echo ""

# ============================================
# FacadeMLP Specific Tests
# ============================================

echo -e "${BLUE}Group: FacadeMLP - Weight Access${NC}"
...
echo -e "${BLUE}Group: FacadeMLP - Bias Access${NC}"
...
run_test \
    "Set bias in model" \
    "$FACADE_BIN set-bias --model=$TEMP_DIR/basic.json --layer=0 --neuron=0 --value=0.3 --save=$TEMP_DIR/bias_mod.json" \
    ""

check_file_exists \
    "Bias-modified model file created" \
    "$TEMP_DIR/bias_mod.json"

echo ""

echo -e "${BLUE}Group: FacadeMLP - Layer Information${NC}"

run_test \
    "Get layer information" \
    "$FACADE_BIN layer-info --model=$TEMP_DIR/multilayer.json --layer=0" \
    "Layer"

run_test \
    "Facade predict on model" \
    "$FACADE_BIN predict --model=$TEMP_DIR/multilayer.json --input=0.1,0.2,0.3" \
    "Output:"

echo ""

echo -e "${BLUE}Group: FacadeMLP - Layer Output${NC}"

run_test \
    "Get individual neuron output" \
    "$FACADE_BIN get-output --model=$TEMP_DIR/basic.json --layer=0 --neuron=0 --input=0.5,0.5" \
    "Output"

echo ""

echo -e "${BLUE}Group: FacadeMLP - Histograms${NC}"

run_test \
    "Generate activation histogram" \
    "$FACADE_BIN histogram --model=$TEMP_DIR/multilayer.json --layer=0 --type=activation" \
    "Histogram"

run_test \
    "Generate weight histogram" \
    "$FACADE_BIN histogram --model=$TEMP_DIR/multilayer.json --layer=0 --type=weight" \
    "Histogram"
...
run_test \
    ...
    "$MLP_BIN create --input=2" \
    "required|error|missing"

run_test \
    "Invalid model file" \
    "$MLP_BIN info --model=/nonexistent/path.json" \
    "error|not found|Error"

run_test \
    "Invalid activation function" \
    "$MLP_BIN create --input=2 --hidden=3 --output=1 --hidden-act=invalid --save=$TEMP_DIR/invalid.json" \
    "error|invalid|Error"
